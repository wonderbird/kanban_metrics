...
class WorkflowSteps:
    ...
    def remove_areas_smaller_than_a_work_item(self, work_items: WorkItems) -> None:
        ...
        for index, workflow_step in enumerate(self.workflow_steps):
            if (
                workflow_step.bounding_rectangle.height > work_items.calculate_largest_height()
                and workflow_step.bounding_rectangle.width
                > work_items.calculate_largest_width()
            ):
